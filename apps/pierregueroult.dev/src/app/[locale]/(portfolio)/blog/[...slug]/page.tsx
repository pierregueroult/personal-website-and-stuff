  ...
  if ('content' in data) {
    return (
      <>
        ...
        <div style={{ marginTop: '40px', padding: '20px', border: '1px solid #e0e0e0', borderRadius: '8px' }}>
              ...
              if (isEmpty) {
                return (
                  <div>
                    ...
                    <p>Aucune recommandation personnalisée pour le moment. Lisez plus d&apos;articles pour obtenir des recommandations personnalisées !</p>
                  </div>
                );
              }
                ...
                <div>
                  ...
                  <ul style={{ listStyle: 'none', padding: 0 }}>
                    {recommendations.map((rec) => (
                      <li key={rec.articleId} style={{ marginBottom: '15px', padding: '10px', background: '#f8f9fa', borderRadius: '4px' }}>
                        <button 
                        ...
                        >
                          ...
                          <br />
                          <small>Score: {rec.finalScore.toFixed(2)} | Comportemental: {rec.behavioralScore.toFixed(2)} | Contenu: {rec.contentScore.toFixed(2)}</small>
                          <br />
                          <small>Tags: {rec.tags.join(', ')}</small>
                          <br />
                          <small style={{ color: '#666' }}>Pourquoi: {rec.reasons.join(', ')}</small>
                        </button>
                      </li>
                    ))}
                  </ul>
            ...
            {({ recommendations, trackRecommendationClick, isEmpty, count }) => {
              ...
              return (
                <div>
                  ...
                  <ul style={{ listStyle: 'none', padding: 0 }}>
                    {recommendations.map((rec) => (
                      <li key={rec.articleId} style={{ marginBottom: '15px', padding: '10px', background: '#fff8f0', borderRadius: '4px' }}>
                        <button 
                          onClick={() => trackRecommendationClick(rec.articleId)}
                          style={{ textAlign: 'left', background: 'none', border: 'none', cursor: 'pointer', width: '100%' }}
                        >
                          <strong>{rec.title}</strong>
                          <br />
                          <small>Similarité: {(rec.similarity || 0).toFixed(2)} | Score: {rec.finalScore.toFixed(2)}</small>
                          <br />
                          <small>Tags: {rec.tags.join(', ')}</small>
                          <br />
                          <small style={{ color: '#666' }}>Pourquoi: {rec.reasons.join(', ')}</small>
